...
На сайте пользователь вводит только `Ключ сохранения`. GitHub token хранится в секретах Worker и не попадает в интерфейс.
...
- `SAVE_API_KEY` - любой придуманный пароль/ключ сохранения для пользователей сайта.
...
npx wrangler secret put SAVE_API_KEY
npx wrangler deploy
```
...
После следующего деплоя поле адреса API исчезнет с сайта, останется только поле `Ключ сохранения`.
...
В правой панели приложения вставьте `Ключ сохранения`. Если `VITE_SAVE_API_URL` еще не задан в GitHub Variables, дополнительно вставьте адрес Worker в поле `Адрес API сохранения`.
...
После изменений нажмите `Сохранить справочник`. Используется тот же `Ключ сохранения`.
