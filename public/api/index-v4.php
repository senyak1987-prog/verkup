...
require_once __DIR__ . DIRECTORY_SEPARATOR . 'bitrix-sync-v4.php';
...
try {
    if ($method === 'GET' && $path === '/health') {
        json_response(['ok' => true, 'storage' => 'beget', 'build' => '2026-07-08-bitrix-runtime-auth-v4']);
    }
    ...
    if (($method === 'GET' || $method === 'POST') && $path === '/bitrix/event') {
        ...
        try {
            if ($dealId !== '') {
                ...
                if (stripos($eventName, 'DELETE') === false && trim((string)array_get(bitrix_runtime_auth_context(), 'token', '')) !== '') {
                    $techSpecImport = fetch_bitrix_deal_tech_spec_files_cached($dealId, true, true);
                    $result['techSpecImport'] = [
                        'status' => array_get($techSpecImport, 'status', ''),
                        'fileCount' => array_get($techSpecImport, 'fileCount', 0),
                        'imageCount' => array_get($techSpecImport, 'imageCount', 0),
                        'import' => array_get($techSpecImport, 'import', null),
                    ];
                }
