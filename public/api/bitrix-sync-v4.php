...
function sync_bitrix_deal($dealId)
{
    ...
    $deal = fetch_bitrix_deal_payload($id);
...
function fetch_bitrix_deal_tech_spec_files($dealId)
{
    ...
    $deal = fetch_bitrix_deal_payload($id);
    if (!is_array($deal) || count($deal) === 0) {
        $cached = cached_bitrix_deal_tech_spec_file_payload($id);
        if (is_array($cached) && count(bitrix_file_array(array_get($cached, 'techSpecFiles', []))) + count(bitrix_file_array(array_get($cached, 'installationFiles', []))) > 0) {
            return $cached;
        }
        $checkedAt = gmdate('c');
        return [
    ...
    if ((int)array_get($status, 'fileCount', 0) <= 0) {
        $cached = cached_bitrix_deal_tech_spec_file_payload($id);
        if (is_array($cached) && count(bitrix_file_array(array_get($cached, 'techSpecFiles', []))) + count(bitrix_file_array(array_get($cached, 'installationFiles', []))) > 0) {
            return $cached;
        }
...
function cached_bitrix_deal_tech_spec_file_payload($dealId)
{
    $id = trim((string)$dealId);
    if ($id === '') return [];

    $data = read_data_file('deals.json');
    $items = array_get($data, 'items', []);
    if (!is_array($items)) return [];

    foreach ($items as $deal) {
        if (!is_array($deal)) continue;
        $dealIdText = first_text(array_get($deal, 'id', ''), array_get($deal, 'number', ''), array_get($deal, 'dealId', ''));
        if ((string)$dealIdText !== $id) continue;

        $techSpecFiles = bitrix_file_array(array_get($deal, 'techSpecFiles', []));
        $installationFiles = bitrix_file_array(array_get($deal, 'installationFiles', []));
        $status = bitrix_tech_spec_status_payload($techSpecFiles, $installationFiles);
        return [
            'success' => true,
            'dealId' => $id,
...
function fetch_bitrix_deal_payload($dealId)
{
    $id = trim((string)$dealId);
    if ($id === '') return [];
    ...
    try {
        ...
        if (is_array($deal) && count($deal) > 0) return merge_bitrix_crm_item_payload($id, $deal);
    } catch (Exception $error) {
        // Some portals expose modern CRM items even when legacy crm.deal.get says "Not found".
    }
    ...
    try {
        ...
        $items = array_get($response, 'result', []);
        if (is_array($items) && isset($items[0]) && is_array($items[0]) && count($items[0]) > 0) {
            return merge_bitrix_crm_item_payload($id, $items[0]);
        }
    } catch (Exception $error) {
        // crm.deal.list is the same API used for the main deals cache; keep falling back.
    }

    return fetch_bitrix_crm_item_payload($id);
}

function merge_bitrix_crm_item_payload($dealId, $deal)
{
    if (!is_array($deal)) return $deal;
    $item = fetch_bitrix_crm_item_payload($dealId);
...
function fetch_bitrix_crm_item_payload($dealId)
{
    try {
        ...
        foreach ($item as $key => $value) {
            ...
            if ($legacyKey !== '') $deal[$legacyKey] = $value;
            $legacyBaseKey = bitrix_legacy_item_key($keyText);
            if ($legacyBaseKey !== '') $deal[$legacyBaseKey] = $value;
        }
        return $deal;
    } catch (Exception $error) {
...
function bitrix_legacy_item_key($key)
{
    static $map = [
        'id' => 'ID',
        'title' => 'TITLE',
    ...
    return (string)array_get($map, (string)$key, '');
}
...
function bitrix_expand_authenticated_url_candidates($urls)
{
    ...
    foreach ($urls as $url) {
        ...
        foreach ([$url, bitrix_url_with_query($url, ['download' => '1'])] as $candidate) {
            ...
            $existingAccessToken = bitrix_url_query_value($candidate, 'access_token');
            if ($auth !== '' && ($existingAccessToken === null || $existingAccessToken === '')) {
                $withAccessToken = bitrix_url_with_query($candidate, ['access_token' => $auth]);
                if ($withAccessToken !== '' && !isset($seen[$withAccessToken])) {
                    $seen[$withAccessToken] = true;
                    $expanded[] = $withAccessToken;
                }
            }
        }
    }
...
{
    $runtime = bitrix_runtime_auth_context();
    $runtimeToken = trim((string)array_get($runtime, 'token', ''));
...
function bitrix_capture_runtime_auth_from_request()
{
    $payloads = [$_POST, $_GET, request_json_if_possible()];
    foreach ($payloads as $payload) {
        $context = bitrix_runtime_auth_from_value($payload);
        if ($context) {
            $GLOBALS['bitrixRuntimeAuth'] = $context;
            return true;
        }
    }
    return false;
...
function bitrix_runtime_auth_context()
{
    $context = array_get($GLOBALS, 'bitrixRuntimeAuth', null);
    if (is_array($context) && trim((string)array_get($context, 'token', '')) !== '') return $context;
    bitrix_capture_runtime_auth_from_request();
    $context = array_get($GLOBALS, 'bitrixRuntimeAuth', null);
    return is_array($context) ? $context : [];
}

function bitrix_runtime_auth_from_value($value)
{
    if (!is_array($value)) return null;

    $auth = array_get($value, 'auth', null);
    if (is_array($auth)) {
        $context = bitrix_normalize_runtime_auth_context(
            first_text(array_get($auth, 'access_token', ''), array_get($auth, 'ACCESS_TOKEN', ''), array_get($auth, 'auth', ''), array_get($auth, 'AUTH_ID', '')),
            first_text(array_get($auth, 'domain', ''), array_get($auth, 'DOMAIN', ''), array_get($auth, 'client_endpoint', ''), array_get($auth, 'server_endpoint', ''))
        );
        if ($context) return $context;
    }

    $context = bitrix_normalize_runtime_auth_context(
        first_text(array_get($value, 'access_token', ''), array_get($value, 'ACCESS_TOKEN', ''), array_get($value, 'auth', ''), array_get($value, 'AUTH_ID', '')),
        first_text(array_get($value, 'domain', ''), array_get($value, 'DOMAIN', ''), array_get($value, 'client_endpoint', ''), array_get($value, 'server_endpoint', ''))
    );
    if ($context) return $context;

    foreach ($value as $item) {
        if (!is_array($item)) continue;
        $context = bitrix_runtime_auth_from_value($item);
        if ($context) return $context;
    }
...
function bitrix_normalize_runtime_auth_context($token, $domain)
{
    ...
    if ($host === '') $host = bitrix_domain();

    $expected = strtolower(bitrix_domain());
    if ($host === '' || ($expected !== '' && $host !== $expected)) return null;
...
function require_bitrix_sync_token()
{
    ...
    if ($token === '') return;
    if (trim((string)array_get(bitrix_runtime_auth_context(), 'token', '')) !== '') return;
...
{
    $runtimeAuth = bitrix_runtime_auth_context();
    $runtimeToken = trim((string)array_get($runtimeAuth, 'token', ''));
    $runtimeDomain = trim((string)array_get($runtimeAuth, 'domain', ''));

    if ($runtimeToken !== '' && $runtimeDomain !== '') {
        $url = 'https://' . $runtimeDomain . '/rest/' . $method . '.json';
        $params = array_merge($params, ['auth' => $runtimeToken]);
    } else {
