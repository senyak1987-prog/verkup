...
export function ProductionMobileApp({
  ...
  const pullLogoOffset = pullRefreshing
    ? 16
    : Math.round(Math.min(PULL_REFRESH_MAX_PX, pullDistance) * 0.18);
  const pullLogoScale = pullRefreshing ? 1.035 : 1 + pullProgress * 0.035;
  const pullWaveAmplitude = Math.sin(pullProgress * Math.PI) * 4;
  const pullWaveOffset = (index: number) => {
    const localProgress = Math.max(0, Math.min(1, pullProgress * 1.5 - index * 0.12));
