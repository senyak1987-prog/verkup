...
export function saleAmountForDeal(deal: Deal, calculation?: DealCalculation, ratio = 0.58) {
  if (deal.source.toLowerCase().includes("агент")) {
    return agentSaleFromCost(cleanCost(calculation), ratio);
  }
  return (Number(deal.saleAmount) || 0) + (Number(deal.installSaleAmount) || 0);
}
